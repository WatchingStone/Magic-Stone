...
/* 定义游戏结束菜单界面的两个按钮RECT范围*/
extern RECT restart_button_rec;
extern RECT end_button_rec;
...
public:
	...
	bool flag_putstorage = 0;	// 用在存储区的标志位，初始置0代表需要输出魔石图像，后续无需输出魔石图像，只需要修改魔石显示数量
	IMAGE* is_monster;	// 魔物区所需的图片
	IMAGE* is_money;	// 金钱区所需的图片
	IMAGE* is_cursor;	// 待部署图片
	IMAGE* is_realcursor;		// 跟随鼠标的待部署魔法阵的光标图片
	IMAGE* is_boom;		// 魔石和魔物爆炸的帧图片
	IMAGE* is_menu;		// 开始菜单和结束菜单
	...
	void putRealCursor(Point mclick, Map mymap, int mc_cat, int mc_ori);	// 跟随光标输出待部署魔法阵
